// Package router (mux) provides a simple Discord message route multiplexer that
...
func (m *Mux) OnMessageCreate(ds *discordgo.Session, mc *discordgo.MessageCreate) {
	...
	if err != nil {
		...
		if err != nil {
			log.Printf("[INFO] Unable to fetch Channel for Message: %s", err)
		} else {
			// Attempt to add this channel into our State
			err = ds.State.ChannelAdd(c)
			if err != nil {
				log.Printf("[INFO] Error updating State with Channel: %s", err)
			}
		}
	}
	...
	// If we're not specifically mentioned we run the help command
	if !ctx.IsDirected {
		if ctx.Content != "!chef" {
			return
		}
	...
	if r != nil {
		log.Printf("[INFO] OnMessageCreate called: !chef %s\n", ctx.Content)

		// Add a reaction to the original message
		err = ds.MessageReactionAdd(mc.Message.ChannelID, mc.Reference().MessageID, os.Getenv("EMOJI_REACTION"))
		if err != nil {
			log.Printf("[INFO] Could not send Discord reaction: %s", err)
		}
